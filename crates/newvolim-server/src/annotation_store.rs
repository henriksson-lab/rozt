...
use newvolim_scene::{
    qupath::{containing_parent, Annotation},
    qupath_geojson,
};
...
impl AnnotationLayer {
    fn new(id: u64, name: String, annotations: Vec<Annotation>) -> Self {
        ...
        Self {
            ...
            dirty: false,
            save_target,
            next_id: assigned.len() as u64 + 1,
        }
    }

    pub(crate) fn add(&mut self, mut item: Annotation) -> Result<Annotation, String> {
        if self.annotations.len() >= 100_000 {
            return Err("annotation layer exceeds 100,000 objects".into());
        }
        ...
        self.next_id = self
            .next_id
            .checked_add(1)
            .ok_or("annotation IDs exhausted")?;
    ...
    pub(crate) fn renest(&mut self) {
        ...
        for item in &mut self.annotations {
            ...
            without.retain(|other| {
                other.id != item.id && !is_descendant(&original, other.id, item.id)
            });
    ...
    pub(crate) fn replace_items(&mut self, items: Vec<Annotation>) -> Result<(), String> {
        ...
        for item in &items {
            ...
            if item.id == 0 || !ids.insert(item.id) {
                return Err("annotation IDs must be unique and nonzero".into());
            }
        }
        if items
            .iter()
            .any(|item| item.parent.is_some_and(|parent| !ids.contains(&parent)))
        {
...
fn validate_annotation(item: &Annotation) -> Result<(), String> {
    ...
    if !valid || count == 0 || count > 65_536 {
        return Err("annotation coordinates must be finite and contain 1–65,536 points".into());
    }
    if item
        .stroke_width
    ...
    if item.measurements.values().any(|value| !value.is_finite()) {
        return Err("measurements must be finite".into());
    }
    Ok(())
}
...
pub struct AnnotationStore {
    datasets: Arc<Mutex<HashMap<String, AnnotationDataset>>>,
}
...
impl AnnotationStore {
    fn with_dataset<T>(
        ...
        dataset: &str,
        root: &Path,
        action: impl FnOnce(&mut AnnotationDataset) -> Result<T, String>,
    ) -> Result<T, String> {
        let mut guard = self
            .datasets
            .lock()
    ...
    pub fn create(
        ...
        self.with_dataset(dataset, root, |state| {
            if state.layers.iter().any(|layer| layer.name == name) {
                return Err("annotation layer name already exists".into());
            }
    ...
    pub fn remove_layer(&self, dataset: &str, root: &Path, id: u64) -> Result<(), String> {
        self.with_dataset(dataset, root, |state| {
            ...
            if state.layers.len() == old_len {
                Err("annotation layer not found".into())
            } else {
                Ok(())
            }
        })
    ...
    pub fn edit<T>(
        ...
        root: &Path,
        id: u64,
        action: impl FnOnce(&mut AnnotationLayer) -> Result<T, String>,
    ) -> Result<T, String> {
        self.with_dataset(dataset, root, |state| {
            let layer = state
                .layers
                ...
                .ok_or("annotation layer not found")?;
            action(layer)
        })
    }
    ...
    fn save_named(
        ...
        self.edit(dataset, root, id, |layer| {
            ...
            let bytes =
                qupath_geojson::write(&layer.annotations).map_err(|error| error.to_string())?;
    ...
    fn save_roi_csv_named(
        ...
        self.edit(dataset, root, id, |layer| {
            ...
            for item in &layer.annotations {
                ...
                if !is_axis_aligned_roi(item) {
                    flattened += 1;
                }
                let a = session
                    .voxel_point_physical_f64([x0, y0, item.plane.z as f64])
                    .map_err(|error| error.to_string())?;
                let b = session
                    .voxel_point_physical_f64([x1, y1, item.plane.z as f64 + item.z_extent as f64])
                    .map_err(|error| error.to_string())?;
                writer
                    .write_record([
                        format!("roi_{}", item.id),
                        a[0].min(b[0]).to_string(),
                        a[1].min(b[1]).to_string(),
                        a[2].min(b[2]).to_string(),
                        (a[0] - b[0]).abs().to_string(),
                        (a[1] - b[1]).abs().to_string(),
                        (a[2] - b[2]).abs().to_string(),
                        item.plane.t.to_string(),
                        item.t_extent.to_string(),
                        item.label.clone(),
                    ])
                    .map_err(|error| error.to_string())?;
            }
    ...
    pub fn save_to(
        ...
        session: &LocalSession,
        target: &str,
    ) -> Result<AnnotationSaveReport, String> {
        let (group, name) = target
            .split_once('/')
            .ok_or("target must be annotations/<name> or tables/<name>")?;
        let name = safe_name(name)?;
    ...
    pub fn replace_from_geojson(
        ...
        if bytes.len() > MAX_ANNOTATION_BYTES {
            return Err("GeoJSON exceeds 128 MiB".into());
        }
    ...
    pub fn roi_tables(root: &Path) -> Result<Vec<RoiTableSummary>, String> {
        ...
        for entry in fs::read_dir(dir).map_err(|error| error.to_string())? {
            ...
            if !matches!(
                attrs.get("type").and_then(serde_json::Value::as_str),
                Some("roi_table" | "masking_roi_table")
            ) {
                continue;
            }
            let backend = roi_backend(&attrs).to_string();
            tables.push(RoiTableSummary {
                name,
                supported: matches!(backend.as_str(), "csv" | "json" | "parquet" | "anndata"),
                backend,
            });
        }
    ...
    pub fn import_roi_table(
        ...
        if !matches!(
            ...
            return Err("table is not an ROI table".into());
        }
        ...
        let rows = match backend {
            ...
            other => {
                return Err(format!(
                    "ROI table backend {other} is not supported by this importer"
                ))
            }
        };
        let annotations = rows
            .into_iter()
            .map(|row| roi_row_to_annotation(&row, session, scale))
        ...
        self.with_dataset(dataset, root, |state| {
            ...
            while state.layers.iter().any(|layer| layer.name == layer_name) {
                layer_name = format!("{candidate}_{suffix}");
                suffix += 1;
            }
            let id = state.next_layer_id;
            state.next_layer_id += 1;
...
fn table_attributes(path: &Path) -> Result<serde_json::Value, String> {
    ...
    if v3.is_file() {
        let metadata: serde_json::Value =
            serde_json::from_slice(&fs::read(v3).map_err(|error| error.to_string())?)
                .map_err(|error: serde_json::Error| error.to_string())?;
        Ok(metadata.get("attributes").cloned().unwrap_or_default())
    } else {
        let v2 = path.join(".zattrs");
        if v2.is_file() {
            serde_json::from_slice(&fs::read(v2).map_err(|error| error.to_string())?)
                .map_err(|error: serde_json::Error| error.to_string())
        } else {
...
impl RoiScale {
    fn from_attributes(attrs: &serde_json::Value) -> Result<Option<Self>, String> {
        let Some(ours) = attrs.get("omezarr_viewer") else {
            return Ok(None);
        };
        let Some(zyx) = ours.get("world_pixel_size_zyx") else {
            return Ok(None);
        };
        let zyx: [f64; 3] = serde_json::from_value(zyx.clone())
            .map_err(|_| "ROI table has invalid world_pixel_size_zyx")?;
        ...
        if !seconds.is_finite() || seconds <= 0.0 {
            return Err("ROI table time scale must be positive and finite".into());
        }
        Ok(Some(Self {
            voxel_xyz: [zyx[2], zyx[1], zyx[0]],
            seconds,
        }))
    }
}
...
fn roi_rows_from_json(bytes: &[u8]) -> Result<Vec<RoiRow>, String> {
    ...
    let stringify = |value: &serde_json::Value| {
        value
            .as_str()
            .map(str::to_string)
                    ...
                    columns
                        ...
                        .filter_map(|(key, values)| {
                            values
                                ...
                                .map(|value| (key.clone(), stringify(value)))
                        })
                        .collect()
                })
                .collect())
...
fn roi_row_to_annotation(
    row: &RoiRow,
    session: &LocalSession,
    scale: Option<RoiScale>,
) -> Result<Annotation, String> {
    let number = |name: &str| -> Result<f64, String> {
        row.get(name)
            .ok_or_else(|| format!("ROI table lacks {name}"))?
            .parse::<f64>()
    ...
    if row.contains_key("__voxel_x") {
        ...
        if [x, y, z].iter().any(|value| !value.is_finite()) {
            return Err("AnnData spatial position must be finite".into());
        }
    ...
    let (lx, ly, lz) = (
        number("len_x_micrometer")?,
        number("len_y_micrometer")?,
        number("len_z_micrometer")?,
    );
    if [x, y, z, lx, ly, lz].iter().any(|value| !value.is_finite())
        || [lx, ly, lz].iter().any(|value| *value < 0.0)
    {
    ...
    let (a, b) = if let Some(scale) = scale {
        ...
        (
            session
                .physical_point_voxel_xyz_f64([x, y, z])
                .map_err(|error| error.to_string())?,
            session
                .physical_point_voxel_xyz_f64([x + lx, y + ly, z + lz])
                .map_err(|error| error.to_string())?,
        )
    };
    let geometry = if (a[0] - b[0]).abs() < 1e-9 && (a[1] - b[1]).abs() < 1e-9 {
        ...
        newvolim_scene::qupath::Geometry::rect(a[0], a[1], b[0], b[1])
    };
    let z_start = a[2].round().max(0.0) as i32;
    let z_extent = (b[2] - a[2]).abs().round().max(0.0) as u32;
    Ok(Annotation {
        geometry,
        plane: newvolim_scene::qupath::Plane::at(
            z_start,
            (number("t_second").unwrap_or(0.0) / scale.map_or(1.0, |scale| scale.seconds)).round()
                as i32,
        ),
        z_extent,
        t_extent: (number("len_t_second").unwrap_or(0.0) / scale.map_or(1.0, |scale| scale.seconds))
            .round()
            .max(0.0) as u32,
        label: row
            .get("class")
...
fn write_roi_group_metadata(root: &Path, name: &str) -> Result<(), String> {
    ...
    let mut parent_metadata: serde_json::Value = if parent_file.exists() {
        serde_json::from_slice(&fs::read(&parent_file).map_err(|error| error.to_string())?)
            .map_err(|error: serde_json::Error| error.to_string())?
    } else if v3 {
    ...
    let listed = if v3 {
        &mut parent_metadata["attributes"]["tables"]
    } else {
        &mut parent_metadata["tables"]
    };
    let mut names = listed.as_array().cloned().unwrap_or_default();
    if !names.iter().any(|value| value.as_str() == Some(name)) {
        names.push(serde_json::json!(name));
    }
    *listed = serde_json::Value::Array(names);
    fs::write(
        &parent_file,
        serde_json::to_vec_pretty(&parent_metadata).map_err(|error| error.to_string())?,
    )
    .map_err(|error| error.to_string())?;
    ...
    if v3 {
        fs::write(
            ...
            serde_json::to_vec_pretty(
                &serde_json::json!({"zarr_format":3,"node_type":"group","attributes":attrs}),
            )
            .map_err(|error| error.to_string())?,
        )
        .map_err(|error| error.to_string())?;
    } else {
        fs::write(parent.join(".zgroup"), b"{\"zarr_format\":2}")
            .map_err(|error| error.to_string())?;
        fs::write(set.join(".zgroup"), b"{\"zarr_format\":2}")
            .map_err(|error| error.to_string())?;
        fs::write(
            set.join(".zattrs"),
            serde_json::to_vec_pretty(&attrs).map_err(|error| error.to_string())?,
        )
        .map_err(|error| error.to_string())?;
    }
    Ok(())
...
fn write_group_metadata(root: &Path, name: &str) -> Result<(), String> {
    ...
    let listed = if v3 {
        &mut parent_metadata["attributes"]["annotations"]
    } else {
        &mut parent_metadata["annotations"]
    };
    let mut names = listed.as_array().cloned().unwrap_or_default();
    if !names.iter().any(|value| value.as_str() == Some(name)) {
        names.push(serde_json::json!(name));
...
fn safe_name(name: &str) -> Result<String, String> {
    ...
    {
        return Err(
            "layer name must be 1–120 characters without path separators or controls".into(),
        );
    }
    Ok(name.into())
}
...
fn prepare_child_directory(
    ...
    for file in [
        parent.join(".zattrs"),
        parent.join(".zgroup"),
        parent.join("zarr.json"),
        child.join(".zattrs"),
        child.join(".zgroup"),
        child.join("zarr.json"),
        child.join("annotations.geojson.tmp"),
        child.join("table.csv.tmp"),
    ] {
...
mod tests {
    ...
    fn layers_nest_edit_save_and_reload_without_reusing_ids() {
        ...
        let layer = store.create("d", root.path(), "cells".into()).unwrap();
        let parent = store
            .edit("d", root.path(), layer.id, |l| {
                l.add(Annotation::rect(0.0, 0.0, 10.0, 10.0, Plane::default()))
            })
            .unwrap();
        let child = store
            .edit("d", root.path(), layer.id, |l| {
                l.add(Annotation::point(5.0, 5.0, Plane::default()))
            })
            .unwrap();
        assert_eq!(child.parent, Some(parent.id));
        store
            .edit("d", root.path(), layer.id, |l| l.remove(parent.id))
            .unwrap();
        store
            .edit("d", root.path(), layer.id, |l| {
        ...
        let third = store
            .edit("d", root.path(), layer.id, |l| {
                l.add(Annotation {
                    geometry: Geometry::Point([6.0, 6.0]),
                    ..Annotation::default()
                })
            })
            .unwrap();
        ...
        let attrs: serde_json::Value = serde_json::from_slice(
            &fs::read(root.path().join("annotations/cells/.zattrs")).unwrap(),
        )
        .unwrap();
        assert_eq!(attrs["coordinate_space"]["units"], "pixel");
        let index: serde_json::Value =
            serde_json::from_slice(&fs::read(root.path().join("annotations/.zattrs")).unwrap())
                .unwrap();
    ...
    fn v3_save_declares_the_annotation_group_and_coordinate_space() {
        ...
        let store = AnnotationStore::default();
        let layer = store
            .create("v3", root.path(), "cell regions".into())
            .unwrap();
        store.save("v3", root.path(), layer.id).unwrap();
        let index: serde_json::Value =
            serde_json::from_slice(&fs::read(root.path().join("annotations/zarr.json")).unwrap())
                .unwrap();
        assert_eq!(
            index["attributes"]["annotations"],
            serde_json::json!(["cell regions"])
        );
        let metadata: serde_json::Value = serde_json::from_slice(
            &fs::read(root.path().join("annotations/cell regions/zarr.json")).unwrap(),
        )
        .unwrap();
    ...
    fn roi_csv_export_uses_physical_spacing_and_reports_flattened_polygons() {
        ...
        for level in ["0", "1", "2"] {
            ...
            fs::copy(
                source.join(level).join("zarr.json"),
                root.path().join(level).join("zarr.json"),
            )
            .unwrap();
        }
        ...
        let annotation = Annotation {
            ...
            plane: Plane::at(2, 0),
            label: "mitotic, cell".into(),
            ..Annotation::default()
        };
        store
            .edit("v3", root.path(), layer.id, |layer| layer.add(annotation))
            .unwrap();
        let report = store
            .save_roi_csv("v3", root.path(), layer.id, &session)
            .unwrap();
        assert_eq!(report.flattened, 1);
        let mut reader =
            csv::Reader::from_path(root.path().join("tables/regions_roi/table.csv")).unwrap();
        ...
        assert_eq!(
            AnnotationStore::roi_tables(root.path()).unwrap()[0].backend,
            "csv"
        );
        let imported = store
            .import_roi_table("v3", root.path(), &session, "regions_roi")
            .unwrap();
        ...
        let saved = store
            .save_to("v3", root.path(), layer.id, &session, "annotations/curated")
            .unwrap();
        assert_eq!(saved.format, "geojson");
        assert_eq!(saved.flattened, 0);
        assert!(root
            .path()
            .join("annotations/curated/annotations.geojson")
            .is_file());
        assert_eq!(
            store.layers("v3", root.path()).unwrap()[0].save_target,
            "annotations/curated"
        );
        let saved = store
            .save_to("v3", root.path(), layer.id, &session, "tables/regions_flat")
            .unwrap();
    ...
    fn roi_table_aliases_and_recorded_pixel_scale_follow_the_source_viewer() {
        assert_eq!(
            roi_backend(&serde_json::json!({"backend":"experimental_csv_v1"})),
            "csv"
        );
        assert_eq!(
            roi_backend(&serde_json::json!({"backend":"experimental_json_v1"})),
            "json"
        );
        assert_eq!(
            roi_backend(&serde_json::json!({"backend":"experimental_parquet_v1"})),
            "parquet"
        );
        assert_eq!(
            roi_backend(&serde_json::json!({"backend":"anndata_v1"})),
            "anndata"
        );
        ...
        let row = RoiRow::from([
            ("x_micrometer".into(), "20".into()),
            ("y_micrometer".into(), "30".into()),
            ("z_micrometer".into(), "40".into()),
            ("len_x_micrometer".into(), "10".into()),
            ("len_y_micrometer".into(), "20".into()),
            ("len_z_micrometer".into(), "4".into()),
            ("t_second".into(), "6".into()),
            ("len_t_second".into(), "4".into()),
