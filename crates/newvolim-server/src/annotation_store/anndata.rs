...
pub(super) fn roi_rows_from_anndata(root: &Path, name: &str) -> Result<Vec<RoiRow>, String> {
    ...
    let shape = x.shape();
    if shape.len() != 2 {
        return Err("AnnData X must have two dimensions".into());
    }
    let (count, width) = (shape[0] as usize, shape[1] as usize);
    if count > 100_000 {
        return Err("ROI table exceeds 100,000 objects".into());
    }
    let names = read_strings(&store, &format!("{prefix}/var/_index"))?;
    if names.len() != width {
        return Err("AnnData var/_index width does not match X".into());
    }
    let values = read_numbers(&store, &x_path)?;
    if values.len() != count.saturating_mul(width) {
        return Err("AnnData X length does not match its shape".into());
    }
    let mut rows = (0..count)
        .map(|index| {
            names
                .iter()
                .enumerate()
                .map(|(column, name)| (name.clone(), values[index * width + column].to_string()))
                .collect::<RoiRow>()
        })
        .collect::<Vec<_>>();
            ...
            if shape.len() == 2 && shape[0] as usize == count && (shape[1] == 2 || shape[1] == 3) {
                ...
                for (index, row) in rows.iter_mut().enumerate() {
                    ...
                    row.insert(
                        "__voxel_z".into(),
                        if width == 3 {
                            values[index * width + 2].to_string()
                        } else {
                            "0".into()
                        },
                    );
    ...
    if let Ok(obs) = Group::open(store.clone(), &format!("{prefix}/obs")) {
        ...
        for column in columns {
            ...
            let decoded = read_categorical(&store, &path)
                .or_else(|_| read_strings(&store, &path))
                .or_else(|_| {
                    read_numbers(&store, &path)
...
fn read_numbers(store: &Arc<FilesystemStore>, path: &str) -> Result<Vec<f64>, String> {
    ...
    macro_rules! widen {
        ($type:ty) => {
            array
                .retrieve_array_subset_elements::<$type>(&subset)
...
mod tests {
    ...
    fn reads_numeric_x_and_categorical_obs() {
        ...
        for path in [
            ...
            if path == "/tables/foreign/obs" {
                builder.attributes(
                    serde_json::json!({"column-order":["class"]})
                        .as_object()
                        .unwrap()
                        .clone(),
                );
        ...
        codes
            .store_array_subset_elements::<i8>(&ArraySubset::new_with_shape(vec![1]), &[1])
            .unwrap();
