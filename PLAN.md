...
5. **Operator migration.** Retain the explicit portable floor-nearest API. Add a separate,
   ...
   builder; choosing the affine path for admitted pages is left to the scheduler's selection
   logic. Evidence in STAGE0 "Resample, the transform-compatible half".
6. **Desktop/server selection.** Select the portable route only for admitted packets and retain
   ...
   orthogonal routes and falls back otherwise; the server never selects it, calling only the
